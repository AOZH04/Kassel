(() => {
    ...
    function initSliders() {
        ...
        const roomSliders = document.querySelectorAll(".room");
        if (roomSliders.length) roomSliders.forEach((room => {
            const slider = room.querySelector(".room__slider");
            const prev = room.querySelector(".room__prev");
            const next = room.querySelector(".room__next");
            const pagination = room.querySelector(".room__pagination");
            if (!slider) return;
            new core(slider, {
                modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
                observer: true,
    ...
    document.addEventListener("watcherCallback", (function(e) {
        ...
        if (entry.isIntersecting) {
            ...
            if (targetElement.classList.contains("animtop")) !targetElement.classList.contains("fadeInUp") ? targetElement.classList.add("fadeInUp") : null;
    ...
    script_tabs.forEach((tab => {
        const headers = tab.querySelectorAll(".tabs_header");
        const contents = tab.querySelectorAll(".tabs_content");
        headers.forEach(((header, index) => {
            header.addEventListener("click", (() => {
                headers.forEach((item => item.classList.remove("is_active")));
                contents.forEach((item => item.classList.remove("is_active")));
                header.classList.add("is_active");
                contents[index].classList.add("is_active");
            }));
        }));
    }));
    const roomTexts = document.querySelectorAll(".room_text");
    if (roomTexts.length) roomTexts.forEach((text => {
        const button = document.createElement("span");
        const room = text.closest(".room");
        const collapsedHeight = 0;
        button.className = "room_more";
        button.textContent = "Подробнее...";
        text.style.maxHeight = collapsedHeight + "px";
        text.insertAdjacentElement("afterend", button);
        button.addEventListener("click", (e => {
            e.stopPropagation();
            if (text.classList.contains("is_active")) {
                text.classList.remove("is_active");
                if (room) room.classList.remove("is_active");
                text.style.maxHeight = collapsedHeight + "px";
                button.textContent = "Подробнее...";
            } else {
                text.classList.add("is_active");
                if (room) room.classList.add("is_active");
                text.style.maxHeight = text.scrollHeight + "px";
                button.textContent = "Скрыть";
            }
        }));
        window.addEventListener("resize", (() => {
            if (text.classList.contains("is_active")) text.style.maxHeight = text.scrollHeight + "px";
        }));
        document.addEventListener("click", (e => {
            if (!text.contains(e.target) && !button.contains(e.target)) {
                text.classList.remove("is_active");
                if (room) room.classList.remove("is_active");
                text.style.maxHeight = collapsedHeight + "px";
                button.textContent = "Подробнее...";
